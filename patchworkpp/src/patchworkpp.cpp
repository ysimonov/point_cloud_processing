...
void PatchWorkpp::flushPatches(std::vector<Zone> &czm)
{
    ...
    for (size_t k = 0; k < params_.num_zones; ++k)
    {
        ...
        {
            for (size_t j = 0; j < params_.num_sectors_each_zone[k]; ++j)
            {
                czm[k][i][j].clear();
            }
        }
    ...
    if (params_.verbose)
        std::cout << "\033[1;31m"
                  << "PatchWorkpp::flushPatches() - Flushed patches successfully!"
                  << "\033[0m" << std::endl;
}

void PatchWorkpp::estimatePlane(const std::vector<PointXYZ> &ground)
{
...
void PatchWorkpp::extractInitialSeeds(const int zone_idx, const std::vector<PointXYZ> &p_sorted,
                                      std::vector<PointXYZ> &init_seeds, double th_seed)
{
    ...
    for (size_t i = 0; i < p_sorted.size(); ++i)
    {
        ...
        {
            init_seeds.emplace_back(p_sorted[i]);
        }
    }
}
...
void PatchWorkpp::estimateGround(Eigen::MatrixXf cloud_in)
{
    ...
    if (params_.enable_RNR)
        removeReflectedNoise(cloud_in);

    clock_t t1 = clock();

    // 2. Concentric Zone Model (CZM)
    flushPatches(ConcentricZoneModel_);
    ...
    unsigned int concentric_idx = 0;

    centers_.clear();
    normals_.clear();

    double t_flush = static_cast<double>(t1_1 - t1);
    double t_czm = static_cast<double>(t2 - t1_1);
    double t_sort = 0.0;
    double t_pca = 0.0;
    double t_gle = 0.0;
    double t_revert = 0.0;
    ...
    for (size_t zone_idx = 0; zone_idx < params_.num_zones; ++zone_idx)
    {

        auto zone = ConcentricZoneModel_[zone_idx];

        for (size_t ring_idx = 0; ring_idx < params_.num_rings_each_zone[zone_idx]; ++ring_idx)
        {
            for (size_t sector_idx = 0; sector_idx < params_.num_sectors_each_zone[zone_idx]; ++sector_idx)
            {
                ...
                centers_.emplace_back(PointXYZ(pc_mean_(0), pc_mean_(1), pc_mean_(2)));
                normals_.emplace_back(PointXYZ(normal_(0), normal_(1), normal_(2)));
                ...
                const double line_variable = singular_values_(1) != 0 ? singular_values_(0) / singular_values_(1)
            ...
            if (!candidates.empty())
            {
                ...
                {
                    revertTemporalGround(ringwise_flatness, candidates, concentric_idx);
                }
                else
                {
    ...
    updateElevationThreshold();
    updateFlatnessThreshold();
    ...
    if (params_.verbose)
    {
        std::cout << "Time taken : " << time_taken_ / double(1000000)
                  << "(sec) ~ "
                  //  << t_flush / double(1000000)  << "(flush) + "
                  << t_czm / double(1000000) << "(czm) + " << t_sort / double(1000000) << "(sort) + "
                  << t_pca / double(1000000) << "(pca) + " << t_gle / double(1000000) << "(estimate)" << std::endl;
...
void PatchWorkpp::updateElevationThreshold()
{
    for (size_t i = 0; i < params_.num_rings_of_interest; ++i)
    {
        if (update_elevation_[i].empty())
            continue;

        double update_mean = 0.0, update_stdev = 0.0;
        findMeanAndStandardDeviation(update_elevation_[i], update_mean, update_stdev);
        if (i == 0)
        {
        ...
        // if (params_.verbose) std::cout << "elevation threshold [" << i << "]: " << params_.elevation_thr[i] <<
        // std::endl;
...
void PatchWorkpp::updateFlatnessThreshold()
{
    for (size_t i = 0; i < params_.num_rings_of_interest; ++i)
    {
        ...
        double update_mean = 0.0, update_stdev = 0.0;
        findMeanAndStandardDeviation(update_flatness_[i], update_mean, update_stdev);
        params_.flatness_thr[i] = update_mean + update_stdev;

        // if (params_.verbose) { std::cout << "flatness threshold [" << i << "]: " << params_.flatness_thr[i] <<
        // std::endl;
        // }
...
void PatchWorkpp::removeReflectedNoise(Eigen::MatrixXf &cloud_in)
{
    if (cloud_in.cols() < 4)
    {
    ...
    for (int i = 0; i < cloud_in.rows(); i++)
    {
        ...
        {
            cloud_nonground_.emplace_back(PointXYZ(cloud_in.row(i)(0), cloud_in.row(i)(1), cloud_in.row(i)(2)));
    ...
    if (params_.verbose)
        std::cout << "PatchWorkpp::removeReflectedNoise() - Number of Noises : " << cnt << std::endl;
}

void PatchWorkpp::revertTemporalGround(std::vector<double> ring_flatness,
                                       std::vector<patchwork::RevertCandidate> candidates, unsigned int concentric_idx)
{
    if (params_.verbose)
        std::cout << "\033[1;34m"
    ...
    findMeanAndStandardDeviation(ring_flatness, mean_flatness, stdev_flatness);
...
// For adaptive
void PatchWorkpp::extractPiecewiseGround(const int zone_idx, const std::vector<PointXYZ> &src,
                                         std::vector<PointXYZ> &dst, std::vector<PointXYZ> &non_ground_dst)
{
    ...
    {
        for (size_t i = 0; i < params_.num_iter; ++i)
        {
            extractInitialSeeds(zone_idx, src_wo_verticals, ground_pc_, params_.th_seeds_v);
            ...
            if (zone_idx == 0 && normal_(2) < params_.uprightness_thr)
            {
                ...
                for (auto point : src_tmp)
                {
                    ...
                    double distance = findPointToPlaneDistance(point, normal_, d_);

                    if (abs(distance) < params_.th_dist_v)
                    {
                        non_ground_dst.emplace_back(point);
                    }
                    else
                    {
                        src_wo_verticals.emplace_back(point);
                    }
                }
            }
            else
...
double PatchWorkpp::findPointToPlaneDistance(PointXYZ p, Eigen::VectorXf normal, double d)
{
    return normal(0) * p.x + normal(1) * p.y + normal(2) * p.z + d;
}

void PatchWorkpp::findMeanAndStandardDeviation(std::vector<double> vec, double &mean, double &stdev)
{
    if (vec.size() <= 1)
        return;

    auto vec_size = vec.size();
    mean = std::accumulate(vec.begin(), vec.end(), 0.0) / static_cast<double>(vec_size);

    for (size_t i = 0; i < vec_size; ++i)
    {
        stdev += std::pow((vec[i] - mean), 2);
    }
    stdev /= (vec_size - 1);
...
void PatchWorkpp::pc2czm(const Eigen::MatrixXf &src, std::vector<Zone> &czm)
{
    ...
    for (int i = 0; i < num_rows; i++)
    {
        ...
        {
            cloud_nonground_.emplace_back(PointXYZ(x, y, z));
        }
    }
    if (params_.verbose)
        std::cout << "\033[1;33m"
